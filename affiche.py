...
for i in ["derivee_centree.txt", "derivee_droite.txt", "derivee_gauche.txt"]:
...
plt.title('Trois méthodes de dérivées, erreur sinus', fontsize=8)
plt.xlabel("x")                                   # <== variable d'abscisse
plt.ylabel("y")
plt.savefig("erreur_dd_dg_dc_sinus") # sauvegarde du fichier.
ax.grid()
ax.show()
